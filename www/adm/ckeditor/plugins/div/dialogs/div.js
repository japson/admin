	...
	function divDialog( editor, command ) {
		...
		return {
			...
			contents: [ {
				...
				elements: [ {
					...
					children: [ {
						...
						setup: function( element ) { console.log('---1');
					...
					/*{
						...
						label: 'Класс',//editor.lang.div.styleSelectLabel,
						'default': '',
						items: [
						 [ '<none>', '' ],
						 [ 'Bold', 'b' ],
			...
			onOk: function() {
				...
				//	if(i >= 1){
					//	console.log(containers[ i ].getAttribute( 'class' ));
					!containers[ i ].getAttribute( 'class' ) && containers[ i ].removeAttribute( 'class' );
				//	}
				}
